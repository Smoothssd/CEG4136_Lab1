...
__global__ void updateForestKernel(int* d_forest, int* d_burnTime, int* d_new_forest, int* d_new_burnTime, int d_N, float spreadProbability, unsigned long seed) {
...
void initializeForest() {
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            ...
            burnTime[i * N + j] = 0;  // Aucun arbre ne brûle au départ // No tree is burning at the start
        }
    }
    ...
    for (int fire = 0; fire < FIRE_START_COUNT && !availablePositions.empty(); fire++) {
        ...
        burnTime[fireX * N + fireY] = BURN_DURATION;  // Définir le temps de combustion // Set the burn duration
    }
...
void updateForestCUDA() {
    ...
    updateForestKernel << <numBlocks, threadsPerBlock >> > (d_forest, d_burnTime, d_new_forest, d_new_burnTime, N, spreadProbability, seed);
    cudaCheckError();
    ...
    printf("CUDA: updateforestKernel took %.3f ms\n", middleTime);
    ...
    // calculate time
    float copyTime;
    cudaEventElapsedTime(&copyTime, middle, stop);
    printf("CUDA: cuda memcpy took %.3f ms\n", copyTime);
...
void updateForestSequential() {
    ...
    std::vector<int> newForest = forest;  // Copie la forêt actuelle // Copy the current forest
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            if (forest[i * N + j] == 2) {  // Si l'arbre est en feu // If the tree is on fire
                burnTime[i * N + j] -= 200;  // Réduire le temps de combustion // Reduce the burning time

                // Vérifier si le feu est éteint // Check if the fire is out
                if (burnTime[i * N + j] <= 0) {
                    newForest[i * N + j] = 3;  // Marquer l'arbre comme brûlé // Mark the tree as burned
                }
                else {
            ...
            // Si un arbre brûle encore, continuer la simulation // If a tree is still burning, continue the simulation
            if (forest[i * N + j] == 2) {
                allBurnedOut = false;
            }
        }
    ...
    forest = newForest;  // Mettre à jour la forêt avec la nouvelle copie // Update the forest with the new copy
